...
Rather than retrying the SAME mirror five times, 
this rotates through config.OVERPASS_MIRRORS on each retry attempt, 
so a temporarily broken mirror gets skipped in favor of a working one.
"""
...
def query_with_retry(query, max_retries=6, base_delay_sec=6):
    """
    Runs an Overpass query, rotating across config.OVERPASS_MIRRORS on each retry attempt. 
    Raises immediately (no retry) only on OverpassBadRequest, 
    which means the query syntax itself is invalid.
